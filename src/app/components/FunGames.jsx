...
import { ArrowRight, Send, Check, Heart, MessageCircle, Sparkles, RotateCcw, ArrowLeft, Lock } from "lucide-react"
...
const sendTGUpdate = async (qNum, question, selected, reason) => {
    const text = `💌 *Priyanshi's Choice (Q${qNum})*\n\n*Q:* ${question}\n*Choice:* ${selected}\n*Her Reply to Your Reaction:* ${reason || "Kuch nahi boli"}`
    try {
...
export default function FunGames({ onComplete }) {
    ...
    // Naya Flow: start -> playing -> confirm -> reply -> finished
    const [gameState, setGameState] = useState("start") 
    const [selectedOpt, setSelectedOpt] = useState(null)
    const [otherText, setOtherText] = useState("")
    const [reasonText, setReasonText] = useState("") // Ab ye uske reply ke liye use hoga
    ...
    // 2. Lock the answer and show Mayank's thought + Reply box
    const handleLockAnswer = () => {
        setGameState("reply")
    }

    // 3. Submit Answer & Her Reply to TG, then Next Q
        ...
        <div className={`min-h-screen flex flex-col items-center justify-center p-4 ${bgBase} text-white font-sans relative overflow-hidden`}>
            ...
            <div className="w-full max-w-[380px] z-10">
                <AnimatePresence mode="wait">
                    ...
                    {/* 2. CONFIRMATION SCREEN (Are you sure?) */}
                    {gameState === "confirm" && (
                    ...
                    {/* 3. MAYANK'S REPLY + TEXTBOX SCREEN */}
                    {gameState === "reply" && (
                        <motion.div key="reply" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className={`p-6 text-center ${puffyCard}`}>
                            ...
                            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} 
                                className={`w-full p-5 rounded-2xl shadow-[inset_4px_4px_8px_#111b25,inset_-4px_-4px_8px_#25394f] mb-6 bg-[#1A2A3C]`}
                            >
                            ...
                            {/* Textarea for her reply to your reaction */}
